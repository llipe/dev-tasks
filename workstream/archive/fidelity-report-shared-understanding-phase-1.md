...
*Evidence:* `git grep -l -E "docs/product\.md|docs/tech\.md" -- .claude .github .kiro` → 32 files; `git grep -l "Fallback, one release cycle"` → 3. `test/unit/foundation-docs-naming.test.ts:126-130`.
...
*Evidence:* `core/distribution/doctor.ts` `checkFoundationDocNames`; `bin/dev-tasks.ts:244-245`; `test/unit/distribution-doctor.test.ts` "fails and names both documents, both new names, and the command (PRD AC-23)".
...
*Evidence:* `grep -n runbooks .claude/skills/activity-init/SKILL.md` → one hit, line 157. `test/unit/skill-parity-init.test.ts`, "confirms the SIMPLICITY.md owner and thresholds with the user (AC-7)".
...
*Evidence:* `git grep -n -E "product-context\.md" main -- CHANGELOG.md` → empty; at `HEAD` → line 20. `SCAN_ROOTS` at `test/unit/foundation-docs-naming.test.ts:24-39`.
