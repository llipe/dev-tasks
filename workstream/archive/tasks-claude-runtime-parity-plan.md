...
**Theme:** every finding here has one root cause. The `.claude` tree's _contracts_ are at parity with Kiro and Copilot; its _runtime bindings_ are not. Enforcement Claude expresses through a consumer-owned file (`.claude/settings.json`, `CLAUDE.md`) is undeliverable, while Kiro expresses the same things through managed paths and ships them working. Task 1 is the unblocker: tasks 2 and 6 produce nothing a consumer can use until it lands.
...
- [ ] 6.0 Cost controls: subagent models and permission allowlist — [#174](https://github.com/llipe/dev-tasks/issues/174)
  ...
  > **Fix (round 3 re-audit, D-3):** A second independent re-audit found the SAME class of bug in `Bash(pnpm run lint:*)`: the literal string `pnpm run lint:fix` starts with the literal string `pnpm run lint` (no space before the colon), so this entry also silently pre-approved the file-mutating `lint:fix` script, with zero backstop (`git-guard.sh` has no pnpm/npm rules at all). It also found that the round-2 regression helper (`allowlistEntryMatches`) required a literal space after the prefix, which is why it caught the space-separated `git branch -D` case but missed the colon-glued `pnpm run lint:fix` case — corrected to a true raw string-prefix match (`command.startsWith(prefix)`) with no boundary requirement, matching Claude Code's actual behavior. `Bash(pnpm run lint:*)` was dropped from `permissions.allow` in both settings files, same removal approach as `git branch`. A full sweep of every remaining entry (`git status`/`diff`/`log`/`rev-parse`, `pnpm run test`/`typecheck`/`format:check`/`audit`, `gh pr view`/`gh issue view`) against every `package.json` script and `gh --help` output found no further collision: `format:check` is already the longer/safer prefix (doesn't collide with `format`'s `--write`), `test:*` collides only with the read-only `test:unit`/`test:integration`, and `gh pr view`/`gh issue view` have no mutating flags. A new generic sweep test (`isWriteCapableScript` + a `pnpm run <x>:*` vs. every write-capable `package.json` script check) now catches this whole class of bug mechanically going forward. See `workstream/fidelity-report-174.md` (D-3) for the full drift writeup.
...
- [x] 7.0 Tool-declaration parity test — **contains a decision point** — [#175](https://github.com/llipe/dev-tasks/issues/175)
  ...
  > Note: `test/unit/qa-engineer-parity.test.ts:162` and `test/unit/researcher-parity.test.ts:139` assert only that a `tools:` line _exists_, never which tools. Nothing mechanically catches an agent whose prompt requires a capability its frontmatter withholds. The invariant — _every agent whose prompt instructs it to invoke a subagent must declare `Task`_ — is four lines and would have caught the `developer` defect at authoring time.
  >
...
- [x] 8.0 Installed-state parity test (depends on 1.0, 2.0, 3.0) — [#176](https://github.com/llipe/dev-tasks/issues/176)
  ...
  > Note: every existing parity test asserts files exist _in this repository_. None asserts that `install --profile claude` produces an install functionally equivalent to `install --profile kiro`. That is the exact blind spot all four delivery gaps fell through — each platform's files were present and correct in-tree, and only the installed result diverged.

  - [x] 8.1 Complete `test/integration/install-parity.test.ts`: install each profile into a separate temp dir and compare _enforcement surface_, not file counts
...
- [x] 10.0 Close the MCP bypass and establish the durable gate (depends on 9.0) — [#178](https://github.com/llipe/dev-tasks/issues/178)
  ...
  - [x] 10.1 Write a test asserting the hook matcher covers the mutating GitHub MCP tool names as well as `Bash`; confirm it fails
...
- [x] 11.0 Never route around a blocked guard — behavioural rules — [#179](https://github.com/llipe/dev-tasks/issues/179)
  ...
  - [x] 11.4 Add an Error Handling row to `.claude/commands/planner.md`: _merge command blocked by git-guard_ → report the block verbatim, mark the story blocked, write the checkpoint, ask the user. Never attempt an alternative merge path
