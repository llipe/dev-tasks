...
- **Impact:** Minor
- **Intent:** Intended
- **Evidence:** `core/exit-codes.ts` names exit code 4 `AuthError`, but spec §6.7 defines code 4 as "Catalog validation errors." The repo's existing convention (see `test/unit/exit-codes.test.ts`, which asserts numeric values only) is that `ExitCode` member _names_ are generic labels and the _numeric values_ are the spec contract. `adapters/cli/validate-component.ts` follows this existing convention and documents the mapping explicitly in a code comment (`ExitCode.AuthError (4) if invalid ... per spec section 6.7`).
