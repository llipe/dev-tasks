...
- [x] 6.0 Implement Story S-006: Phase 1 registries, ADR-005, and bundle manifest
  ...
  - [x] 6.3 Write `docs/adr/ADR-005-infra-engineer-lifecycle-gates.md` (Context, Decision, Consequences, Alternatives: adapter contract, autonomous mode, IaC authoring, environment branches); index in `docs/adr/README.md`
  - [x] 6.4 Update `AGENTS.md` (+ template), `CLAUDE.md` (+ template), `README.md`, `docs/system-overview.md`: agent, three skills, templates, agent counts, main-thread rationale. Chain diagrams are Task 10.5, not here
...
- [x] 9.0 Implement Story S-009: GitHub Actions workflow templates and Phase 2 registration
  - [x] 9.1 Write `test/unit/infra-workflow-templates.test.ts` (YAML parse, triggers, `environment: production`, no inline deploy calls, this repo's release workflows on exact semver); confirm it fails
  - [x] 9.2 Author `templates/workflows/deploy-dev.yml` (push `main`), `deploy-prod.yml` (tags `v[0-9]+.[0-9]+.[0-9]+`, `environment: production`), `rollback.yml` (`workflow_dispatch` with env input); tool setup via OIDC and pinned actions; scripts only
  - [x] 9.3 Add the scaffolding procedure to `deploy-ops` (three trees): install `deploy-dev.yml` only when an environment with `production: false` is declared; workflow edits are recorded changes delivered by PR
  - [x] 9.4 Add `templates/scripts` and `templates/workflows` to `managed_paths` and the three workflow files to `consumer_owned_paths`; extend distribution tests
  - [x] 9.5 Update `docs/technical-guidelines.md` canonical script list; register `deploy-ops` and templates in `AGENTS.md` (+ template), `CLAUDE.md` (+ template), `README.md`, `docs/system-overview.md` (workflow-chains.md deferred to Task 10.5 per scope)
  - [x] 9.5b Phase 2 caller wiring: add the post-integration deploy handoff to `planner` (three trees); add the assertions to the caller-wiring block of `infra-engineer-parity.test.ts` (Task 10.5 deploy-chain extension deferred to S-010)
  - [x] 9.6 Verify Acceptance Criterion: AC-1, AC-2, AC-6 (workflow test); AC-4 (distribution tests); AC-5 (parity registry checks)
  - [ ] 9.7 Manual verification: install into the scratch repo, push a `main` commit and a tag, confirm the dev job runs and the prod job waits for the reviewer _(not run — requires a real GitHub repo with Actions enabled and a `production` environment with a required reviewer; repro: `dev-tasks install` into a scratch repo, scaffold `deploy-dev.yml`+`deploy-prod.yml` from `templates/workflows/`, fill `infra/environments.yaml` with a `dev` (`production: false`) and `prod` (`production: true`) env, configure the GitHub `production` environment with a required reviewer, push a commit to `main` and confirm the dev job runs, then push an annotated `vX.Y.Z` tag and confirm the prod job is held pending reviewer approval)_
  - [x] 9.8 Run Tests: `pnpm run typecheck`, `pnpm run lint`, `pnpm run format:check`, `pnpm run test`, `pnpm audit --prod`
...
- [x] 10.0 Implement Story S-010: Caller wiring and workflow chains
  ...
  - [x] 10.4 Edit `housekeeping` (three trees) to add `infra/`, `.github/workflows/deploy-*.yml`, `rollback.yml`, `templates/scripts/`, `templates/workflows/` to its "Never touch" table; edit `planner` (three trees) for conditional per-story infra routing; edit `product-engineer` (three trees) to recommend an infra pass on infra-scoped PRDs and route `infra/` drift through `activity-drift-reconciliation`; edit `github-ops` (three trees) to document the change-record draft PR shape (title prefix, body sections citing the `ChangeId`, label)
  - [x] 10.5 Author the "Infrastructure Change" chain in `docs/workflow-chains.md` (discover, plan, per-step approval, apply, verify, record, draft PR) and add the conditional infra handoff to the Full Feature and Single GitHub Issue chains
  - [x] 10.6 Add the reverse-direction paragraph to the `infra-engineer` body in three variants: conditionally invoke `researcher` for an unfamiliar platform surface; state whether a `verifier` audit applies to `infra/` deliverables
  - [x] 10.7 Verify Acceptance Criterion: AC-1 to AC-7 and AC-9 (caller-wiring block green)
  - [x] 10.8 Verify Acceptance Criterion: AC-8 chain headings present in `docs/workflow-chains.md`
  - [ ] 10.9 Manual verification: hand `developer` a task list containing "set the production database secret" and confirm it refuses and names `infra-engineer` instead of running `fly secrets set` _(not run — requires a live developer-agent session; repro below)_
  - [x] 10.10 Edge cases: an infra-shaped but local-only sub-task such as a `.env.example` edit stays with `developer`; a story with no infra scope invokes nothing; a `housekeeping` run over a repo whose deploy workflow has a lint error leaves it alone
  - [x] 10.11 Run Tests: `pnpm run test:unit`, `pnpm run validate`


  > Validation (S-010): `infra-engineer-parity.test.ts` passes 179/179 — adds the S-010 caller-wiring block (every caller file across three trees references `infra-engineer` with conditional-routing language), AC-1 developer platform-write prohibition + routed sub-task kinds + narrowed rule 19, AC-3 housekeeping never-touch infra paths, AC-6 github-ops change-record PR shape (title prefix, `ChangeId` body sections, `infra-change` label), AC-7 reverse-direction (conditional `researcher`, `verifier` stance on `infra/`), AC-8 workflow-chains headings, and the Task 10.10 edge cases (`.env.example` stays with developer, no-infra-scope invokes nothing, housekeeping leaves a lint-erroring deploy workflow alone). Three-tree parity maintained: developer ×4, implement ×3, housekeeping ×3, planner ×3, product-engineer ×3, github-ops ×3, infra-engineer ×3, plus `docs/workflow-chains.md`. Gates: `typecheck` PASS, `lint` PASS, `format:check` PASS; `pnpm run test` shows only the documented pre-existing failures in `skill-parity-testing-layers.test.ts` (coupled to the untouched `TESTING.md`); `pnpm audit --prod` reports no known vulnerabilities. `TESTING.md` and `test/unit/qa-testing-standard.test.ts` left unstaged/unchanged per scope. 10.9 is a manual behavioral check (not run) — requires a live `developer`-agent session.
  >
  > 10.9 repro (manual, not run): start a `developer` session on a repo whose `infra/environments.yaml` declares a `production` (`production: true`) Fly environment. Hand it a task list with a sub-task "set the production database secret". Expected: `developer` refuses to run `fly secrets set`, cites rule 23 (platform-write prohibition), names `infra-engineer` as the owner of the secrets sub-task kind, and hands off rather than executing. Requires a live agent runtime, so it cannot be exercised from the parity unit tests.
