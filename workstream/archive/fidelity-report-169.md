...
- **Evidence:**
  ...
  - Issue #169 body's AC-1 checkbox carries the annotation "(delivery mechanism verified; `PreToolUse` wiring *content* lands with task 2.0 per explicit execution-time scope direction — see PR #180)" — i.e., the issue itself, not just internal docs, discloses the narrowing.
...
- **Consumer-facing functional check (the specific concern raised for this audit):** confirmed non-surprising in the sense that matters most — a consumer installing today gets `.claude/hooks/git-guard.sh` on disk and an empty `PreToolUse`, and the new `checkClaudeHooksWiring` doctor check will name `git-guard.sh` as unwired if the consumer runs `dev-tasks doctor`. The gap is surfaced by name, not silently inert-and-invisible, which is the ADR's stated goal. See D2 for the residual gap in *how* a consumer discovers this.
