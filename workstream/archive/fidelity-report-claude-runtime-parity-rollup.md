...
- `test/integration/install-parity.test.ts` performs a **real install** via the built CLI (`dist/bin/dev-tasks.js`) into fresh temp directories per profile (`claude`, `kiro`, `copilot`, `both`, `all`) and asserts on the _installed output_, not source-tree presence:
  ...
  - Documented asymmetries (command-vs-agent shape for `infra-engineer`/`planner`/`product-engineer`, `verifier` as two commands, merged `planner-resume`) are asserted to hold and a regression guard fails if an _undocumented_ asymmetry widens.
