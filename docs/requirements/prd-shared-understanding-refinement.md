...
| 1.15    | 2026-09-22 | Added the `## Vocabulary` section FR-20/AC-07 requires of every grilled PRD, listing the eight terms of specification §8.7 as `proposed` under the package map's single bounded context. Resolves the self-application gap the Phase 3 test plan records as A-8: this PRD carries `## Decisions`, so §8.3's skip rule does not skip it and `lint` would otherwise report `vocabulary-missing` against it. Phase 3 story S-006 flips these rows to `existing` when it populates the glossary (D-69, D-71). Deviates from D-71 and specification §8.7, which assigned this section to S-006 listing the terms as `existing`; `existing` was unavailable while the glossary was empty, and the walk had already failed this PRD. That deviation is now recorded as D-76, superseding D-71, and the developer-edits-a-product-document route as D-77. | developer                            |
| 1.16    | 2026-09-22 | Phase 3 story S-006 flipped the eight `## Vocabulary` rows from `proposed` to `existing`: `docs/domain/ubiquitous-language.md` now defines all eight, and each term string was checked against its `### <Term>` heading one by one rather than assumed, because the flip matches exactly and a mismatch is silent under `lint` (D-76). A test now asserts that coupling (issue #231 audit finding D-8). Minimal, disclosed developer edit to a `product-engineer`-owned document under D-77.                                                                                                                                                                                                                                                                                                                                                      | developer                            |
...
The eight terms this PRD's own history defines, now `existing` in `docs/domain/ubiquitous-language.md` under the package map's single bounded context (spec §8.7, D-69). They shipped here as `proposed` in S-003, because `existing` was unavailable while the glossary was still empty; S-006 populated the file and flipped them, matched on the exact term string (D-76, superseding D-71). Each `Term` below equals a `### <Term>` heading in the glossary character for character, and a test asserts it — a `proposed` row for a term the glossary does not define leaves `lint` green, so the coupling needs a guard of its own. Forbidden synonyms are recorded only where this PRD's history supplies one (D-72); the glossary is the canonical text for every definition here.

| Term                | Status in glossary | Bounded context                  | Definition (proposals only)                                                                                                              | Forbidden synonyms (proposals only)   |
| ------------------- | ------------------ | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| decision log        | existing           | AI-assisted development workflow | The append-only `workstream/decisions-<feature>.md` table recording every question, recommendation, answer, and resolution of a feature. | none                                  |
| grilling            | existing           | AI-assisted development workflow | The one-question-at-a-time, depth-first interview that must reach a confirmed exit gate before any document is drafted.                  | none                                  |
| exit gate           | existing           | AI-assisted development workflow | The hard stop that requires an empty open list and an explicit user statement of shared understanding before drafting begins.            | none                                  |
| bounded context     | existing           | AI-assisted development workflow | A named boundary within which one term has exactly one meaning; recorded per package in the `docs/tech.md` package map.                  | none                                  |
| package map         | existing           | AI-assisted development workflow | The `docs/tech.md` table mapping each workspace package to its path, purpose, owner, canonical scripts, and bounded context.             | none                                  |
| runbook             | existing           | AI-assisted development workflow | A `docs/runbooks/` document describing a multi-step setup, configuration, or migration procedure that is otherwise held in one head.     | none                                  |
| install-if-absent   | existing           | AI-assisted development workflow | The delivery mode that writes a consumer-owned file only when it is missing and never overwrites an edited copy on a later run.          | none                                  |
| foundation document | existing           | AI-assisted development workflow | A repository-level context document every agent reads: `docs/product.md` and `docs/tech.md`.                                             | product-context, technical-guidelines |
