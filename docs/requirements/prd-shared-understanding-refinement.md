...
A preparatory move (Phase 0) simplifies the documentation foundation every later phase names: `product-context.md` becomes `docs/product.md`, `technical-guidelines.md` becomes `docs/tech.md`, operational procedures gain a home in `docs/runbooks/`, and `housekeeping` keeps that home organized on every run.
...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 0     | Docs foundation: rename `product-context.md` → `docs/product.md` and `technical-guidelines.md` → `docs/tech.md` with a one-release fallback; `docs/runbooks/` with index, frontmatter, and a deterministic structure check under `lint`; `housekeeping` runbook hygiene | Nothing. Runs first because every new skill in Phases 1 to 4 names these documents.               |
...
Phase 0 runs first so the new skills are written against the new names once. Phase 1 delivers most of the value on its own and is the first feature milestone.
...
15. As a maintainer, I want `housekeeping` to flag a runbook that is orphaned, malformed, or stale on every run, so that the runbook set does not rot between incidents.
...
### Docs foundation and runbooks (Phase 0)
...
47. `docs/runbooks/` **MUST** exist in this repository and in every consumer where `dev-tasks` is installed, holding one file per operational procedure named `runbook-<verb>-<object>.md`, plus `docs/runbooks/README.md` as the index. `install` scaffolds the directory, the index, and a template with install-if-absent semantics.
...
49. Every script under `templates/scripts/`, every workflow under `templates/workflows/` and `.github/workflows/`, and every `infra-engineer` change kind **MUST** have a runbook. `infra-engineer` **MUST** produce or update the runbook in the same draft PR that delivers a script or workflow. `technical-writer` owns runbook content accuracy.
50. `housekeeping` **MUST** keep the runbook set organized on every run and **MUST** fail on: an index entry with no file, a file with no index entry, missing or invalid frontmatter, a name that violates the convention, and a `related` entry pointing at a script or workflow that no longer exists. A `last_verified` older than the staleness window (default 90 days, OQ-14) is reported as stale, not failed. The check **MUST** be deterministic and **MUST** run under `lint`, so `validate` covers it.
51. Ownership split: `housekeeping` owns runbook structure (index, frontmatter, naming, links, staleness), `technical-writer` owns runbook content, and neither edits the other's concern. A structural failure that needs content to fix is routed to `technical-writer`.
...
- [ ] AC-22: After Phase 0, no file in `.claude/`, `.github/`, `.kiro/`, `core/`, `adapters/`, `bin/`, `test/`, `docs/`, or the root references `product-context.md` or `technical-guidelines.md`, and a parity test asserts it.
...
| D-10 | Supersedes D-07. One PRD delivered as five milestones: Phase 0 (docs foundation: `product.md`, `tech.md`, runbooks, housekeeping hygiene) runs first, then Phases 1 to 4 in the delivery-phases order.                                                                     |
