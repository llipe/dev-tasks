...
The eight terms this PRD's own history defines, proposed for `docs/domain/ubiquitous-language.md` under the package map's single bounded context (spec §8.7, D-69, D-71). They are `proposed` until Phase 3's approval append lands them in the glossary; Phase 3 story S-006 flips these rows to `existing` in the same step that populates the file. Forbidden synonyms are recorded only where this PRD's history supplies one (D-72).

| Term                | Status in glossary | Bounded context                  | Definition (proposals only)                                                                                                            | Forbidden synonyms (proposals only) |
| ------------------- | ------------------ | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| decision log        | proposed           | AI-assisted development workflow | The append-only `workstream/decisions-<feature>.md` table recording every question, recommendation, answer, and resolution of a feature. | none                                |
| grilling            | proposed           | AI-assisted development workflow | The one-question-at-a-time, depth-first interview that must reach a confirmed exit gate before any document is drafted.                  | none                                |
| exit gate           | proposed           | AI-assisted development workflow | The hard stop that requires an empty open list and an explicit user statement of shared understanding before drafting begins.            | none                                |
| bounded context     | proposed           | AI-assisted development workflow | A named boundary within which one term has exactly one meaning; recorded per package in the `docs/tech.md` package map.                  | none                                |
| package map         | proposed           | AI-assisted development workflow | The `docs/tech.md` table mapping each workspace package to its path, purpose, owner, canonical scripts, and bounded context.             | none                                |
| runbook             | proposed           | AI-assisted development workflow | A `docs/runbooks/` document describing a multi-step setup, configuration, or migration procedure that is otherwise held in one head.     | none                                |
| install-if-absent   | proposed           | AI-assisted development workflow | The delivery mode that writes a consumer-owned file only when it is missing and never overwrites an edited copy on a later run.          | none                                |
| foundation document | proposed           | AI-assisted development workflow | A repository-level context document every agent reads: `docs/product.md` and `docs/tech.md`.                                             | product-context, technical-guidelines |
