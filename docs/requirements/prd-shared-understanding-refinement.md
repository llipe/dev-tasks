...
68. The explainer **MUST NOT** be a file list or a diff restatement. Files are named only where the reader has to go there. Code appears only as a worked example, never as a substitute for explanation (PR-KT PRD FR-3).
...
71. Format: the explainer is Markdown with Mermaid diagrams at `workstream/explain-<issue-or-pr>.md`, linked from the PR body's `## How It Works` section and archived with the workstream (OQ-16). The `github-ops` PR Description Template **MUST** gain the three issue #141 sections (Context / How it works; What changed & why it matters; Examples) at SHOULD level with Examples REQUIRED for user-visible or contract changes, mirrored across the three trees with a parity test, exactly as `workstream/issue-141-pr-teach-team-refinement.md` specifies; the explainer fills them.
72. Depth is proportional: a Short-tier PR (docs, formatting, dependency bump, CI) gets no explainer. The tier rule from the PR-KT PRD (FR-7) applies with its signals derived from the FR-70 change map. Enforcement is advisory in the first release: the verifier **MUST** report a missing or placeholder explainer on a behavioral PR as a finding; it **MUST NOT** block.
...
79. Durable understanding **MUST** be written back: when an explainer or walkthrough states an architectural decision, a business rule, or a domain term that is not yet in `docs/`, the glossary, or the decision log, the skill **MUST** propose the write-back to `technical-writer` (docs), to the glossary via a Vocabulary proposal (FR-20), or to `memo` when available, non-blocking (PR-KT PRD FR-28 to FR-31).
...
- Blocking a PR on explainer completeness in the first release. Advisory findings only; the PR-KT PRD's enforcement tier is deferred.
...
- [ ] AC-27: After Phase 0, `pnpm validate` passes with no `dt` binary, no `dt` module, no `dt` reference in any prompt tree or `AGENTS.md`, and no `pg` peer dependency; a parity test asserts the absence.
...
- [ ] AC-36: The `github-ops` PR Description Template carries the three issue #141 sections identically in all three trees, and the parity test fails when one tree diverges.
...
- Relates to `docs/requirements/prd-pr-knowledge-transfer.md`: Phase 6 delivers its Phase 1 MVP (issue #141) as specified and re-scopes its Phase 2 (skill, change map, write-back) without `dt` and without blocking enforcement. That PRD's changelog records the hand-over; its full tiered contract and enforcement remain deferred.
...
- OQ-16: Explainer format. You asked for HTML with diagrams. Recommended: Markdown with Mermaid at `workstream/explain-<id>.md`, because it renders inside the PR on GitHub and in every editor, needs no hosting, and follows the pattern every PRD and spec in this repository already uses. An HTML export can be added later through the existing html-lite scaffold if a hosted page proves worth it.
- OQ-17: Trace renderer in the first release, or skill-only narration first? Recommended: ship the renderer as a `dev-tasks trace render` subcommand from the start. The mapping from log line to diagram message is mechanical, scenario traces are too long for an LLM to draw reliably, and it keeps the repository's deterministic-first rule.
- OQ-18: Logger wrapper versus adopting OpenTelemetry. Recommended: wrap the project's existing logger (pino, winston, console) once at the boundary. OpenTelemetry is the right answer for distributed services and the wrong one for learning inside a monorepo; the trace contract can map onto OpenTelemetry attributes later without changing the walkthrough.
