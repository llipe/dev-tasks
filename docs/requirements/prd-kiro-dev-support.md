...
- **Agent-selector structure (was Open Question 1):** Each of the 6 delegated agents (`developer`, `technical-writer`, `housekeeping`, `github-ops`, `ux-engineer`, `black-box-tester`) **MUST** be both directly selectable as Kiro's active agent for standalone/interactive use *and* invocable as a delegated subagent by `product-engineer`/`planner` — mirroring `.claude/`'s dual-role pattern (e.g. `developer` as both command and subagent). FR #2 is updated accordingly. The spec **MUST** confirm the exact Kiro mechanics for a single agent file serving both roles (or whether two files per agent are needed).
