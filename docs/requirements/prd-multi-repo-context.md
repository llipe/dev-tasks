...
1. **Extraction (`init` extended):** derive `schema.md`, OpenAPI, AsyncAPI (Kafka topic inventory + payloads), and a `component.json` manifest from a repository's code, declaring the provenance and confidence of every extracted artifact and field.
2. **Catalog:** aggregate each repo's `component.json` into a meta-repo, generate a flat routing `index.yaml`, model end-to-end flows, and validate referential integrity in CI.
...
flowchart LR
    subgraph Repo["Component repo (existing)"]
        ...
        EX --> CY[component.json + contracts + schema.md]
    end
...
**Delivery phases** (extraction precedes catalog, a change from the earlier draft, because hand-populating 20 `component.json` files is the bottleneck that would otherwise sink the project):
...
| O6  | Populate the catalog by extraction, not by hand              | ≥80% of `component.json` fields derived automatically           |
...
| Component repositories (~20)          | Each gains a root `component.json`, `contracts/` (OpenAPI/AsyncAPI), a generated `docs/schema.md`, an `AGENTS.md`, and a `.dev-tasks/` pin + manifest.              |
...
5. (RF-05, P0) It **MUST** derive `component.json` from the extraction and the repository.
...
13. (RF-20, P0) Each repo **MUST** declare its metadata in a root `component.json`.
14. (RF-21, P0) The meta-repo **MUST** aggregate `component.json` files into `catalog/components/` via CI.
...
28. (RF-36, P0) Repo fetch **MUST** be sparse: `component.json`, `docs/`, `contracts/`.
...
- **`component.json`** — per-repo manifest with `provides`/`consumes`, datastores, docs paths, `aliases`, and a `_provenance` block recording `source`, `confidence`, and per-field hashes (for manual-edit detection).
...
5. Given an existing Node/TS repo, `dt extract all` derives `component.json`, `schema.md`, OpenAPI, and a Kafka topic inventory, and every artifact/field carries `source` and `confidence`.
...
8. A `component.json` `consumes` entry that does not resolve to a `provides` in the catalog fails `dt catalog validate` with an error exit code.
...
| --------------------------------------------- | -------- | ------------------------------- |
| Components with a valid `component.json`      | 0/20     | 20/20                           |
| `component.json` fields derived automatically | 0%       | ≥80%                            |
...
- The context layer is expected to be commoditized in 12-18 months; standard interfaces (JSON, MCP, `component.json` as a bridge format) keep each layer replaceable without touching the others.
