...
Autonomous documentation maintenance agent. Keeps `/docs` artifacts aligned with the current state of the codebase. Creates ADRs in `/docs/adr/` when technical guidelines change. Never modifies application code.

**When to use:** Invoked automatically by `developer` before completing any story/issue. Can also run standalone to audit documentation drift.
