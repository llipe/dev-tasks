/**
 * Manual-scope init orchestration for `dt init --components`.
 *
 * Orchestrates: meta-repo pin → freshness check → resolve components →
 * fetch per-component repos → assemble bundle → emit session.lock.json.
 *
 * Spec: §5.6 + §6.5 (RF-31, RF-38, RF-39).
 */
...
import { buildSessionLock, writeSessionLock, type SessionLock } from "./session-lock.js";
