...
Synchronize documentation with the codebase.
...
- **Activity-Based Instructions table** — every steering document in `.kiro/steering/` **MUST** have a row; no row **MAY** reference a deleted file.
- **Domain-Specific Instructions table** — same rule for scoped steering documents.
- **Agents table** — every agent file in `.kiro/agents/` **MUST** have a row.
