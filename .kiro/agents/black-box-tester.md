...
Build a black-box compliance test plan from a spec or story.
...
When complete, hand off to `developer` for implementation, then use Validate Mode after implementation to verify compliance.
...
Chains: intake → requirement extraction → evidence collection → reporting & publication.
