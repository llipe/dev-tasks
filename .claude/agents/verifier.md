...
2. **Fidelity Audit Gate (post-implement):**
   ...
   - This gate is additive and non-blocking to PR/issue completion. It is triggered automatically and mandatorily by `developer` and `planner` (see the `developer` and `planner` subagents); drift findings route to `product-engineer`'s `activity-drift-reconciliation` skill.
