...
| **activity-integration-test-implementation** | `skills/activity-integration-test-implementation/` | Author integration tests (Layer 2.5) against real databases — local via testcontainers/docker-compose/Supabase CLI, remote via testing environments | `qa-engineer` |
| **activity-e2e-test-implementation**     | `skills/activity-e2e-test-implementation/`     | Author Playwright end-to-end tests from verifier scenario tables. Covers auth, state reset, CI config, and scenario-to-spec traceability | `qa-engineer` |
| **activity-contract-validation**         | `skills/activity-contract-validation/`         | Validate API contracts against implementation using dt verify (contract-diff, impact, drift). Detects breaking changes and spec-to-code staleness | `qa-engineer` |
